import { validateEmail } from './email'

const ServiceName = 'foo'

const RequiredError: Error = {
  name: 'ValidationError',
  message: 'email-required',
}

const InvalidError: Error = {
  name: 'ValidationError',
  message: 'email-invalid',
}

const LengthError: Error = {
  name: 'ValidationError',
  message: 'email-length',
}

const CharacterError: Error = {
  name: 'ValidationError',
  message: 'email-reserved',
}

describe('email validator', () => {
  it('throws when email is not provided', () => {
    expect(() => validateEmail(ServiceName, { email: undefined })).toThrow(
      RequiredError
    )
    expect(() => validateEmail(ServiceName, { email: null })).toThrow(
      RequiredError
    )
  })

  it('throws when email is not a string', () => {
    /// @ts-expect-error checking failing functionality
    expect(() => validateEmail(ServiceName, { email: 42 })).toThrow(
      InvalidError
    )
    expect(() =>
      /// @ts-expect-error checking failing functionality
      validateEmail(ServiceName, { email: false })
    ).toThrow(InvalidError)
    expect(() =>
      /// @ts-expect-error checking failing functionality
      validateEmail(ServiceName, { email: () => 421 })
    ).toThrow(InvalidError)
    expect(() =>
      /// @ts-expect-error checking failing functionality
      validateEmail(ServiceName, { email: 423.22 })
    ).toThrow(InvalidError)
  })

  it('throws when email is an empty string', () => {
    expect(() => validateEmail(ServiceName, { email: '' })).toThrow(
      InvalidError
    )
  })

  it('throws when email is longer than 254 characters', () => {
    expect(() =>
      validateEmail(ServiceName, {
        email:
          '',
      })
    ).toThrow(LengthError)
  })

  it('throws when email contains reserved characters', () => {
    expect(() =>
      validateEmail(ServiceName, { email: 'foobar$@example.com' })
    ).toThrow(CharacterError)
    //
    expect(() =>
      validateEmail(ServiceName, {
        email: 'fodad#fa2$17sdf(fa|fa!d@example.com',
      })
    ).toThrow(CharacterError)
    //
    expect(() =>
      validateEmail(ServiceName, { email: "fodad'obar@example.com" })
    ).toThrow(CharacterError)
    //
    expect(() =>
      validateEmail(ServiceName, { email: 'fodad"obar@example.com' })
    ).toThrow(CharacterError)
    //
    expect(() =>
      validateEmail(ServiceName, {
        email: 'fodadconsole.log("obar");@example.com',
      })
    ).toThrow(CharacterError)
    //
    expect(() =>
      validateEmail(ServiceName, {
        email: 'foda_adwk@ex_ample.com',
      })
    ).toThrow(CharacterError)
    //
    expect(() =>
      validateEmail(ServiceName, {
        email: 'fo..da_adwk@example.com',
      })
    ).toThrow(CharacterError)
  })
})
