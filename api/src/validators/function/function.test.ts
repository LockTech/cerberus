...
describe('function validator', () => {
  describe('validateJSONBody', () => {
    it('throws when `body` is defined and not a string', () => {
      ...
      expect(() => validateJSONBody(Service, { headers, body: 1 })).toThrow(BodyInvalid)
      // @ts-expect-error checking failing functionality
      expect(() => validateJSONBody(Service, { headers, body: true })).toThrow(BodyInvalid)
      // @ts-expect-error checking failing functionality
      expect(() => validateJSONBody(Service, { headers, body: false })).toThrow(BodyInvalid)
      // @ts-expect-error checking failing functionality
      expect(() => validateJSONBody(Service, { headers, body: () => 1 })).toThrow(BodyInvalid)
    })
    ...
    it('DOES NOT throw when `body` is undefined', () => {
      // @ts-expect-error checking failing functionality
      expect(() => validateJSONBody(Service, { headers, body: undefined })).not.toThrow(BodyInvalid)
      // @ts-expect-error checking failing functionality
      expect(() => validateJSONBody(Service, { headers, body: null })).not.toThrow(BodyInvalid)
    })
    ...
    it(`DOES NOT throw when \`headers\` have ${FunctionHeaderMediaTypeKey} set to ${FunctionHeaderMediaType}`, () => {
      expect(
        // @ts-expect-error checking failing functionality
        () => validateJSONBody(Service, { body, headers })
      )
    })
  })
